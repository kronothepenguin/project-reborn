...
A single `the` proxy — **exported from the public lingo entry** (the player/runner registers it onto `globalThis` when booting a movie, feature 008 boundary) — exposes the documented `the`-property surface: movie, player, sound, key, mouse, and system state, read through the active singletons (`_movie`, `_player`, `_sound`, `_key`, `_mouse`, `_system`), with read-only vs read/write classification per the properties reference, and with the `the`-function forms (counts and "last X in") callable per the translation contract. Read-only writes are rejected per the docs. It is installed (registered as a global) only by the player runner; without a context activation, no-context singleton defaults serve; when a context is activated, the live-binding slot mechanism (established in 002) makes the same `the` object reflect the active context.
...
**Independent Test**: A vitest suite where (a) `the` imports from the public lingo entry without leaking onto `globalThis`; (b) every property in the doc-derived property table reads a defined value of the documented type; (c) every read-only property rejects a write; (d) read/write properties store and return the new value; (e) the `the`-function forms return documented counts/last-chunk results; (f) after installing an activated context, reads delegate to that context's instances and return to defaults after reset.

**Acceptance Scenarios**:

1. **Given** no context activated, **When** `the` is imported from the public lingo entry, **Then** reads like `the.itemDelimiter`, `the.mouseLoc`, `the.floatPrecision` return defined defaults without error, and importing the entry does NOT leak `the` onto `globalThis` (global registration is the player's job).
...
- **FR-007**: The runtime MUST export a single `the` proxy from the public lingo entry, without requiring an activated context or leaking it onto `globalThis`, exposing the documented `the`-property surface (movie/player/sound/key/mouse/system state and documented global constants). Registering the runtime globals — including `globalThis.the` — is the player runner's responsibility (feature 008 installs all globals before the movie bundle import). *Testable: `the` imports from the lingo entry with no context; `globalThis.the` stays unset at module load.*
...
- **The `the` proxy binding**: the single proxy exported from the `@/lingo` entry (player/runner registers `globalThis.the` when booting a movie — feature 008 boundary); reads/writes delegate through the live-binding singleton slots; no-context defaults back it until a context activates; the Score/stage-backed subset returns documented no-op defaults until feature 004.
