...
- [ ] T002 [P] Verify `packages/director/package.json`: `"type": "module"`, exports map (`.` → `./src/index.js`, `./lingo` → `./src/lingo/index.js`, `./browser` → `./src/browser/index.js`), devDependencies `vitest ^4.1.8` + `jsdom ^29.1.1`, script `"test": "vitest run"`. NO CHANGE (verified correct, research.md R3).
...
**Purpose**: Unblock everything in the exact order research.md R4 mandates: (a) US4 scope — remove all stale tests/shims/dead trees so the suite is a clean slate; (b) write the 7 new test files and OBSERVE RED on all of them; (c) US3 scope — repair every broken import so the package imports cleanly; (d) observe the intermediate state (entry-points/point/rect green, behavior tests still red). **No user story work can begin until this phase is complete.**

### Block A — US4 scope: test cleanup (spec FR-009; first executable work)

- [ ] T003 Delete all 132 stale `*.test.js` files in `packages/director/src` — the 9 `__tests__` directories, verified counts: `src/__tests__/` (1: `public-barrels.test.js`), `src/engine/types/__tests__/` (5), `src/lingo/methods/__tests__/` (106), `src/runtime/__tests__/` (3), `src/runtime/package/__tests__/` (1), `src/runtime/player/__tests__/` (4), `src/runtime/player/custom-elements/__tests__/` (1), `src/runtime/subsystems/__tests__/` (3), `src/runtime/syntax/__tests__/` (8). DELETE, never patch or adapt (FR-009: revival = fresh rewrite only). Do not touch any non-test source file.
...
- [ ] T008 [P] Docs touch-up in `packages/director/AGENTS.md`: remove the "Test Shims" section and all `__test-shims__` references (including the command cheat-sheet note "jsdom env + __test-shims__"); fix stale architecture references (`src/runtime/objects/`, `src/runtime/methods/` → `src/engine/objects/`, `src/lingo/methods/`, `src/engine/types/` per the post-refactor layout). No other AGENTS.md content changes.
...
### Block C — US3 scope: stabilization (import-path repair against the current layout — no file moves, no rewrites; 42 source files from research.md R3)

- [ ] T018 [P] Fix `packages/director/src/lingo/index.js`: rewrite all 107 broken method imports from `../runtime/methods/<X>.js` to `../lingo/methods/<X>.js` (all 107 targets verified to exist); REMOVE the 5 creator re-exports (`color`, `list`, `point`, `propList`, `rect` — deferred to 006, contract §Removed); ADD 5 class exports from `../engine/types/{color,list,prop-list,point,rect}.js`; keep the already-resolving `export * from "../runtime/constants.js"` and `export * from "../runtime/syntax/index.js"` (and the singletons re-export from `../runtime/singletons.js`) unchanged.
- [ ] T019 [P] Fix `packages/director/src/browser/index.js`: change `../runtime/creators/movie.js` → `../runtime/package/movie.js` and `../runtime/creators/cast.js` → `../runtime/package/cast.js`; REMOVE the dangling `defineMovie`/`defineCast` exports (their modules were deliberately deleted by the 001 refactor, refactor.md 211–227; contract §Removed); remaining exports (`createContext`, `destroyContext`, `resetSingletons`, `registerCustomElements`, `_createMovie`, `movie`, `cast`) unchanged.
- [ ] T020 [P] Fix `packages/director/src/runtime/index.js`: 13 × `./objects/<X>.js` → `../engine/objects/<X>.js` and 5 × `./types/<X>.js` → `../engine/types/<X>.js`; leave already-resolving imports (`./constants.js`, `./singletons.js`, `./context.js`) untouched.
- [ ] T021 [P] Fix `packages/director/src/runtime/context.js`: 7 × `./objects/<X>.js` → `../engine/objects/<X>.js`.
- [ ] T022 [P] Fix `packages/director/src/runtime/singletons.js`: 7 × `./objects/<X>.js` → `../engine/objects/<X>.js`.
- [ ] T023 [P] Fix `packages/director/src/runtime/player/cast-loader.js`: `../objects/cast-library.js` → `../../engine/objects/cast-library.js`.
- [ ] T024 [P] Fix `packages/director/src/runtime/syntax/the-proxy.js`: `../objects/cast-library.js` → `../../engine/objects/cast-library.js`.
- [ ] T025 [P] Fix the singleton import in 12 method files under `packages/director/src/lingo/methods/`: `../singletons.js` → `../../runtime/singletons.js` in `alert.js`, `appMinimize.js`, `beep.js`, `beginRecording.js`, `breakLoop.js`, `callFrame.js`, `castLib.js`, `cursor.js`, `delay.js`, `externalParamName.js`, `externalParamValue.js`, `flushInputEvents.js`.
- [ ] T026 [P] Fix the singleton import in the remaining 12 method files under `packages/director/src/lingo/methods/`: `../singletons.js` → `../../runtime/singletons.js` in `go.js`, `goLoop.js`, `goNext.js`, `goPrevious.js`, `halt.js`, `idleLoadDone.js`, `insertFrame.js`, `marker.js`, `quit.js`, `sound.js`, `sprite.js`, `stopEvent.js`.
- [ ] T027 [P] Fix the types import in 10 method files under `packages/director/src/lingo/methods/`: `../types/<X>.js` → `../../engine/types/<X>.js` in `color.js`, `flashToStage.js`, `list.js`, `listP.js`, `makeSubList.js`, `max.js`, `min.js`, `point.js`, `propList.js`, `rect.js`. IMPORT FIXES ONLY — the 5 creator modules (`color`, `list`, `point`, `propList`, `rect`) stay dormant with no behavior work (006 owns them; contract §Removed).
- [ ] T028 [P] Fix `packages/director/src/lingo/methods/ilk.js`: 5 × `../types/*.js` → `../../engine/types/*.js` and 7 × `../objects/*.js` (including `../objects/index.js`) → `../../engine/objects/*.js`.
...
**Checkpoint**: Foundation ready — all three public entries import with zero module-resolution errors (SC-004); the US3 regression test (entry-points) is green; the 4 behavior-pending test files are red with recorded evidence; no stale tests remain; no package.json change (SC-005/SC-007 partially gated).
...
**Independent Test**: `pnpm --filter @project-reborn/director test` — the 5 files under `packages/director/src/engine/types/__tests__/` pass, exercising every documented operation with doc-defined expected results, including the quickstart scenarios 2–6 surface audit.
...
**Independent Test**: `pnpm --filter @project-reborn/director test` — `packages/director/src/runtime/__tests__/constants.test.js` passes, asserting all 11 values (quickstart scenario 7).
...
- [ ] T036 [US2] Fix `packages/director/src/runtime/constants.js`: change `BACKSPACE` to `String.fromCharCode(8)` (`"\b"`, Lingo character semantics per R2 — the constants.txt keyCode columns 51/3/36/49/48 are the JS-syntax ALTERNATIVE, not the constant values); do NOT touch the other 10 constants (all verified doc-conformant; `ENTER` = chr(3) is already correct); keep the verbatim JSDoc blocks.
...
| US3 — Stabilization (P1) | T018–T028 (all 42 files from research.md R3) | T016 + T029 (entry-points regression), T039 (static audit); acceptance scenarios 1–5 |
...
- [ ] T039 [P] Static import audit (SC-007): grep `packages/director/src` for stale-path patterns — `../runtime/methods/`, `../runtime/creators/`, `./objects/`, `./types/` (from `runtime/`), `../types/`, `../objects/`, `../singletons.js` — zero remaining references in any module reachable from the three public entries.
- [ ] T040 Final gate: run `pnpm --filter @project-reborn/director test` → exactly 7 test files in `src`, all green, zero pre-existing failures (SC-006); confirm `src/__test-shims__/`, `src/engine/packaging/`, and `architecture/` are absent; commit the feature branch state. **Checkpoint: feature 002 complete.**
...
T017 (RUN → RED ×7 on module resolution) — red evidence #1
   ↓
T018–T028 (US3 scope: stabilization repairs; all [P] — different files)
   ↓
...
- **Foundational (Phase 2)**: Depends on Setup. INTRAPHASE ORDER IS MANDATORY: cleanup (T003–T009) → write the 7 tests (T010–T016) → observed red (T017) → stabilization repairs (T018–T028) → intermediate observed red (T029). No user story work may start before this completes.
- **User Stories (Phases 3–4: US1, US2)**: Depend on Foundational completion — a clean suite (US4, T003–T009), a resolvable barrel (US3, T018–T028), and the pre-written red test files (T010–T016, red observed at T017/T029) are all required before the US1/US2 behavior fixes can be validated. US1 and US2 are mutually independent (disjoint files: `src/engine/types/*.js` vs `src/runtime/constants.js`) and run sequentially here in story priority order.
...
- **User Story 2 (P1)** — Constants: Starts after Foundational. No dependency on US1 — its single implementation file (`src/runtime/constants.js`) is disjoint from the US1 files, so the two stories COULD run in parallel if staffed; executed sequentially here (P1 → P1 order). Its test file exists since Foundational (T015, red at T017/T029).
...
| US3 stabilization (after T017) | T018–T028 | `lingo/index.js`, `browser/index.js`, `runtime/index.js`, `runtime/context.js`, `runtime/singletons.js`, `player/cast-loader.js`, `syntax/the-proxy.js`, 3 disjoint `lingo/methods/` file groups (T025/T026/T027), `ilk.js` |
| US1 type fixes (after T030) | T031, T032, T033 | `engine/types/color.js`, `engine/types/list.js`, `engine/types/prop-list.js` |
| Polish | T038, T039 | quickstart runs vs static import audit |

Notes on the partitioning: T003 (the 132-test sweep) is deliberately NOT `[P]` — it must land before any test write or run — while T004/T005/T006/T007/T008 are parallel with each other. T025/T026 split the 24 singleton-import method files into two review-sized groups; both groups remain disjoint from T027 (types-import group) and T028 (`ilk.js`). Within US1, T031–T033 are disjoint files and parallel; the creator-fix group inside T027 is import-only (see Notes).
...
Task: "src/engine/types/__tests__/color.test.js (T010)"
Task: "src/engine/types/__tests__/list.test.js (T011)"
Task: "src/engine/types/__tests__/prop-list.test.js (T012)"
Task: "src/engine/types/__tests__/point.test.js (T013)"
Task: "src/engine/types/__tests__/rect.test.js (T014)"
Task: "src/runtime/__tests__/constants.test.js (T015)"
Task: "src/__tests__/entry-points.test.js (T016)"
# then, together as the observed-red step: run the test command (T017)

# US3 stabilization batch — all 11 repair tasks launched together (after red evidence):
Task: "lingo/index.js 107 imports + export set (T018)"
Task: "browser/index.js creators + drop defineMovie/defineCast (T019)"
Task: "runtime/index.js barrel (T020)"
Task: "runtime/context.js (T021)"
Task: "runtime/singletons.js (T022)"
Task: "player/cast-loader.js (T023)"
Task: "syntax/the-proxy.js (T024)"
...
2. Complete Phase 2: Foundational (CRITICAL — blocks all stories):
   ...
   - US3 stabilization (T018–T028) — barrel resolves;
...
- **Creator functions deferred to 006** (contract §Removed): `color()`, `list()`, `point()`, `propList()`, `rect()` are NOT exported from `@/lingo` in 002; `rgb()` must not exist. The 5 creator modules in `src/lingo/methods/` receive IMPORT FIXES ONLY (via T027, inside the 10-file types-import group) — they stay dormant with zero behavior work until 006. `symbol()`/`value()`/`ilk()` relationship work is likewise 006-owned; `ilk.js` here gets path fixes only (T028).
- **No silent scope creep**: every source edit in this feature is enumerated above and in plan.md/research.md R3/R5. No file moves (the post-refactor layout is final, research R3); no `package.json` change (verified correct); no new abstractions, factories, or wrappers (KISS/YAGNI); `src/runtime/index.js` is fixed, not enriched.
...
- **Out of scope re-verified**: no changes to `src/engine/objects/`, `src/runtime/package/`, `subsystems/`, `player/` (sources), or `syntax/` sources; no test suites outside `packages/director/` are touched (spec Assumptions); two-active-contexts isolation is a later spec's concern.