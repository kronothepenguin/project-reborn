...
const _store = new GlobalObject();

export const _global = new Proxy(_store, {
  get(target, prop) {
    ...
    return _store._get(prop);
  },
  set(_target, prop, value) {
    if (typeof prop === "symbol") return true;
    _store._set(prop, value);
    return true;
  },
  has(_target, prop) {
    if (typeof prop === "symbol") return false;
    return _store._has(prop);
  },
  ...
  getOwnPropertyDescriptor(_target, prop) {
    ...
    if (!_store._has(prop)) return undefined;
    return { configurable: true, enumerable: true, value: _store._get(prop), writable: true };
  },
});
