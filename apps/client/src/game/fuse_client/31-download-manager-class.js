export default class {
  ...
  queue(tURL, tMemName, ttype, tForceFlag, tDownloadMethod, tRedirectType, tTarget) {
    if (!ilk(tURL, Symbol.for("string"))) {
      return error(this, `Missing or invalid URL: ${tURL}`, Symbol.for("queue"), Symbol.for("major"));
    }
    ...
    if ((this.pOwnDomain != tDownloadDomain) && ((tURL.contains("http://")) || (tURL.contains("https://"))) && !(tURL.contains("://localhost"))) {
      ...
      if (!tAllowCrossDomain) {
        const tPref = getPref("CrossDomainAlert.txt");
        setPref("CrossDomainAlert.txt", `${tURL} ${date()} ${time()}${RETURN}${tPref}`);
        return error(this, `Cross domain download not allowed: ${tURL}`, Symbol.for("queue"), Symbol.for("minor"));
      }
    }
    if (!voidp(this.pTaskQueue[tMemName]) || !voidp(this.pActiveTasks[tMemName])) {
      return error(this, `File already downloading: ${tMemName}`, Symbol.for("queue"), Symbol.for("minor"));
    }
    let tMemNum;
    ...
    if (tMemNum < 1) {
      return error(this, "Failed to create member!", Symbol.for("queue"), Symbol.for("major"));
    } else {
      if (member(tMemNum).type == Symbol.for("bitmap")) {
        member(tMemNum).image = image(1, 1, 8);
      }
    }
    this.pReceivedTasks.add(tMemName);
    const tTempTask = propList("url", tURL, "memNum", tMemNum, "type", ttype, "callback", VOID);
  ...
  registerCallback(tMemNameOrNum, tMethod, tClientID, tArgument) {
    let tTaskData = this.searchTask(tMemNameOrNum);
    if (!tTaskData) {
      if (stringp(tMemNameOrNum)) {
        if (getmemnum(tMemNameOrNum) == 0) {
          return error(this, `Task doesn't exist: ${tMemNameOrNum}`, Symbol.for("registerCallback"), Symbol.for("major"));
        }
      } else {
        if (integerp(tMemNameOrNum)) {
          ...
          return error(this, `Member's name or number expected: ${tMemNameOrNum}`, Symbol.for("registerCallback"), Symbol.for("major"));
        }
      }
      tTaskData = propList("status", Symbol.for("complete"));
    }
    if (!symbolp(tMethod)) {
      return error(this, `Symbol referring to a handler expected: ${tMethod}`, Symbol.for("registerCallback"), Symbol.for("major"));
    }
    if (!objectExists(tClientID)) {
      return error(this, `Object not found: ${tClientID}`, Symbol.for("registerCallback"), Symbol.for("major"));
    }
    if (!getObject(tClientID).handler(tMethod)) {
      return error(this, `Handler not found in object: ${tMethod}`, tClientID, Symbol.for("registerCallback"), Symbol.for("major"));
    }
    switch (tTaskData[Symbol.for("status")]) {
      ...
      case Symbol.for("queue"):
        this.pTaskQueue[tTaskData[Symbol.for("name")]][Symbol.for("callback")] = propList("method", tMethod, "client", tClientID, "argument", tArgument);
        break;
      case Symbol.for("Active"):
        call(Symbol.for("addCallBack"), this.pActiveTasks, tTaskData[Symbol.for("name")], propList("method", tMethod, "client", tClientID, "argument", tArgument));
        break;
    }
    return 1;
  }
  ...
  getLoadPercent(tMemNameOrNum) {
    ...
    if (integerp(tMemNameOrNum)) {
      ...
      if (stringp(tMemNameOrNum)) {
        ...
        return error(this, `Member's name or number expected: ${tMemNameOrNum}`, Symbol.for("getLoadPercent"), Symbol.for("minor"));
      }
    }
    if (this.pReceivedTasks.getOne(tMemName) == 0) {
      return error(this, `Downloaded file not found: ${tMemName}`, Symbol.for("getLoadPercent"), Symbol.for("minor"));
    }
    if (!voidp(this.pActiveTasks[tMemName])) {
      return this.pActiveTasks[tMemName].getProperty(Symbol.for("Percent"));
    } else {
  ...
  print() {
    const tListList = list(this.pActiveTasks, this.pTaskQueue, this.pReceivedTasks);
    for (const tList of tListList) {
      const tListMode = ilk(tList);
  ...
  searchTask(tMemNameOrNum) {
    if (stringp(tMemNameOrNum)) {
      ...
      const tTaskData = propList("name", tMemNameOrNum, "number", getmemnum(tMemNameOrNum), "status", VOID);
      let tResource = this.pTaskQueue[tMemNameOrNum];
      if (!voidp(tResource)) {
      ...
      return error(this, `Referred task not found: ${tMemNameOrNum}`, Symbol.for("searchTask"), Symbol.for("minor"));
    } else {
      if (integerp(tMemNameOrNum)) {
        return this.searchTask(member(tMemNameOrNum).name);
      }
    }
    return error(this, `Member's name or number expected: ${tMemNameOrNum}`, Symbol.for("searchTask"), Symbol.for("minor"));
  }
  ...
  updateQueue() {
    if (this.pActiveTasks.count < getIntVariable("net.operation.count")) {
      if (this.pTaskQueue.count > 0) {
        ...
        const tTaskData = this.pTaskQueue[tTaskName];
        this.pTaskQueue.deleteProp(tTaskName);
        if (tTaskData[Symbol.for("downloadMethod")] == Symbol.for("httpcookie")) {
          this.pActiveTasks[tTaskName] = createObject(getUniqueID(), getClassVariable("httpcookie.instance.class"));
        } else {
          this.pActiveTasks[tTaskName] = createObject(getUniqueID(), getClassVariable("download.instance.class"));
        }
  ...
  removeActiveTask(tMemName, tCallback, tSuccess) {
    ...
    for (let i = 1; i <= this.pActiveTasks.count; i++) {
      if (this.pActiveTasks[i].pMemName == tMemName) {
        if (!tSuccess) {
          this.pLastError = netError(this.pActiveTasks[i].pNetId);
        }
        this.pActiveTasks[i].deconstruct();
        this.pActiveTasks.deleteAt(i);
        this.pCompleteTasks.add(tMemName);
        this.updateQueue();
        break;
      }
    }
    if (!voidp(tCallback)) {
      if (objectExists(tCallback[Symbol.for("client")])) {
        call(tCallback[Symbol.for("method")], getObject(tCallback[Symbol.for("client")]), tCallback[Symbol.for("argument")], tSuccess);
      }
    }
    return 0;
  }
  ...
  recognizeMemberType(tURL) {
    ...
    let tFileType = tURL.char[`${length(tURL) - 5}..${length(tURL)}`];
    tFileType = tFileType.char[`${offset(".", tFileType) + 1}..${length(tFileType)}`];
    tFileType = this.pTypeDefList[tFileType];
    if (!symbolp(tFileType)) {
      error(this, `Couldn't recognize member's type: ${tURL}`, Symbol.for("recognizeMemberType"), Symbol.for("minor"));
