  ...
  return {
    ...
    setKey(tMyKey, tMode) {
      ...
      switch (tMode) {
        ...
        case Symbol.for("artificialKey"): {
          let len = Math.floor(bitAnd(tMyKey, 248) / 8);
          if (len < 20) {
            len = len + 20;
          }
          ...
          for (let i = 0; i <= len - 1; i++) {
            ...
            const tOwn = artificialKey[(Math.abs(tOffset + i) % artificialKey.count) + 1];
            ckey[i + 1] = bitAnd(tGiven ^ tOwn, 32767);
          }
          for (let i = 0; i <= 255; i++) {
    ...
    encipher(tdata) {
      ...
      for (let e = 1; e <= length(tdata); e++) {
        const a = charToNum(tdata.char[e]);
        if (a > 255) {
          tBytes.add(Math.floor((a - (a % 256)) / 256));
          if (a % 256) {
            tBytes.add(a % 256);
          }
          continue;
      ...
      for (let a = 1; a <= tBytes.count; a++) {
        ...
        tCipher = tCipher + tStrServ.convertIntToHex(tBytes[a] ^ d);
      }
      return tCipher;
    },
    ...
    bitshiftright(x, n) {
      return bitOr(Math.floor(x / Math.pow(2, n)), 0);
    },
