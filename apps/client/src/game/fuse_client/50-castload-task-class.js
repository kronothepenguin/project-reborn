export default class {
  ...
  UpdateTaskPercent(tInstancePercent, tFile) {
    ...
    if (this.pTmpLoadCount == this.pCurLoadCount) {
      const tTemp = 1.0 * (this.pTempPercent + this.pLoadedSoFar) / this.pCastcount;
  ...
  addCallBack(tID, tMethod, tClientID, tArgument) {
    if (!symbolp(tMethod)) {
      return error(this, `Symbol referring to handler expected: ${tMethod}`, Symbol.for("addCallBack"), Symbol.for("major"));
    }
    if (!objectExists(tClientID)) {
      return error(this, `Object not found: ${tClientID}`, Symbol.for("addCallBack"), Symbol.for("major"));
    }
    if (!getObject(tClientID).handler(tMethod)) {
      return error(this, `Handler not found in object: ${tMethod}/${tClientID}`, Symbol.for("addCallBack"), Symbol.for("major"));
    }
    if (this.pStatus == Symbol.for("ready")) {
      ...
      if (this.pStatus == Symbol.for("LOADING")) {
        if (voidp(this.pCallBack)) {
          this.pCallBack = list(list(propList("method", tMethod, "client", tClientID, "argument", tArgument)));
        } else {
          this.pCallBack.add(list(propList("method", tMethod, "client", tClientID, "argument", tArgument)));
        }
      }
    }
    return 1;
