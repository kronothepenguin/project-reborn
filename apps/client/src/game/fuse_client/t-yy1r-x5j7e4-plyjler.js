...
import { _global } from "../../director";

export default function () {
  _global._player = _global._player ?? VOID;
  ...
  return {
    ...
    qe2AkKOGGKDTTnd1Nei(tMyKey, tMode, tOtherKey) {
      if (_global._player !== VOID) {
        if (_global._player.traceScript) {
          return 0;
        }
      }
      _global._player.traceScript = 0;
      _movie.traceScript = 0;
      let tMyKeyS = string(tMyKey);
      pR3hu24v5 = list();
      const tKey = list();
      ...
      switch (tMode) {
        ...
        case VOID:
          for (this.q = 0; this.q <= 255; this.q++) {
            tKey[this.q + 1] = charToNum(charOf(tMyKeyS)[(this.q % length(tMyKeyS)) + 1]);
            this.pR3hu24v5[this.q + 1] = this.q;
          }
          break;
        case Symbol.for("artificialKey"): {
          const len = bitAnd(tMyKey, 248) / 8;
          let finalLen = len;
          if (finalLen < 20) {
            finalLen = finalLen + 20;
          }
          ...
          let prevKey = 0;
          const m = 5;
          for (this.q = 0; this.q <= finalLen - 1; this.q++) {
            const tGiven = this.b6(tMyKey, this.q % 32);
            const tOwn = artificialKey[(Math.abs(tOffset + this.q) % artificialKey.length) + 1];
            ckey[this.q + 1] = bitAnd(bitXor(tGiven, tOwn), 32767);
          }
          for (this.q = 0; this.q <= 255; this.q++) {
            tKey[this.q + 1] = ckey[(this.q % finalLen) + 1];
            fakeKey[this.q + 1] = tKey[this.q + 1];
            this.pR3hu24v5[this.q + 1] = this.q;
          }
          break;
        }
        case Symbol.for("new"):
          for (this.q = 0; this.q <= 255; this.q++) {
            tKey[this.q + 1] = this.q;
          }
          for (this.q = 0; this.q <= 1019; this.q++) {
            tKey[(this.q % 256) + 1] = (charToNum(charOf(tMyKeyS)[(this.q % length(tMyKeyS)) + 1]) + tKey[(this.q % 256) + 1]) % 256;
          }
          for (this.q = 0; this.q <= 255; this.q++) {
            this.pR3hu24v5[this.q + 1] = this.q;
          }
        ...
        case Symbol.for("initMUS"): {
          let tModKey = EMPTY;
          let l = 1;
          for (let k = 1; k <= charOf(tMyKeyS).count; k++) {
          ...
          for (this.q = 0; this.q <= 255; this.q++) {
            tKey[this.q + 1] = charToNum(charOf(tModKey)[(this.q % charOf(tModKey).count) + 1]);
            this.pR3hu24v5[this.q + 1] = this.q;
          }
          break;
        }
      ...
      this.j = 0;
      for (this.q = 0; this.q <= 255; this.q++) {
        this.j = (this.j + this.pR3hu24v5[this.q + 1] + tKey[this.q + 1]) % 256;
        const k = this.pR3hu24v5[this.q + 1];
    ...
    zLmj71sZDldCwpaZLbqHds(tdata) {
      ...
      for (let e = 1; e <= length(tdata); e++) {
        let a = charToNum(charOf(tdata)[e]);
        if (a > 255) {
      ...
      const tStrServ = _director.getStringServices();
    ...
    TTF97D0LvibV6X(tdata) {
      ...
      for (let a = 1; a <= length(tdata); a++) {
        ...
        const d = this.pR3hu24v5[((this.pR3hu24v5[this.q + 1] + this.pR3hu24v5[this.j + 1]) % 256) + 1];
        const t = tStrServ.convertHexToInt(charOf(tdata).slice(a, a + 1));
    ...
    jfh2ZSJi5QnANFH() {
      ...
      const tSeed = _director.the.randomSeed;
      _director.the.randomSeed = _director.the.milliSeconds;
      const tLength = tKeyMinLength + Math.abs(random(65536) % tKeyLengthVariation);
      let tTable = EMPTY;
      let tKey = EMPTY;
      for (let j = 1; j <= tLength; j++) {
        const c = charOf(tCharacters)[(random(65536) % tCharacters.length) + 1];
        tTable = tTable + c;
        const c2 = charOf(tCharacters)[(random(65536) % tCharacters.length) + 1];
        tTable = tTable + c2;
        tKey = tKey + c2;
      }
      const tCodedKey = tTable + tKey;
      _director.the.randomSeed = tSeed;
      return tCodedKey;
    },
