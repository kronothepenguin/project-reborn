export default class {
  ...
  deconstruct() {
    return getResourceManager().removeMember(this.pMember.name);
  }

  prepare() {
    this.pMember = member(getResourceManager().createMember(`${this.pProps[Symbol.for("member")]}${the.milliSeconds}${numToChar(random(99))}`, Symbol.for("field")));
    ...
    if (this.pProps[Symbol.for("key")] == EMPTY) {
      ...
      if (textExists(this.pProps[Symbol.for("key")])) {
        this.pMember.text = getText(this.pProps[Symbol.for("key")]);
      } else {
        error(this, `Text not found: ${this.pProps[Symbol.for("key")]}`, Symbol.for("define"), Symbol.for("minor"));
  ...
  draw(tRGB) {
    ...
    the.stage.image.draw(this.pSprite.rect, propList("shapeType", Symbol.for("rect"), "color", tRGB));
  }
}
