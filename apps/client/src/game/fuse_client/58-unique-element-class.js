export default class {
  ...
  define(tProps) {
    ...
    const tMemNum = getResourceManager().getmemnum(this.pProps[Symbol.for("member")]);
    if (tMemNum > 0) {
  ...
  resizeTo(tX, tY, tForcedTag) {
    const tOffX = tX - this.pSprite.width;
    const tOffY = tY - this.pSprite.height;
  ...
  setProperty(tProp, tValue) {
    switch (tProp) {
      ...
      case Symbol.for("member"):
        switch (ilk(tValue)) {
          ...
          case Symbol.for("string"):
            this.pSprite.member = getMember(tValue);
            break;
      ...
      case Symbol.for("depth"):
        this.pDepth = tValue;
        const tImage = this.pimage.duplicate();
  ...
  draw(tRGB) {
    ...
    this.pBuffer.image.draw(this.pBuffer.image.rect, propList("shapeType", Symbol.for("rect"), "color", tRGB));
  }
}
