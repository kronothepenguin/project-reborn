...
export default function () {
  return {
    ...
    construct() {
      ...
      this.pBoundary = rect(0, 0, the.stage.rect.width, the.stage.rect.height).__add__(list([-1000, -1000, 1000, 1000]));
    ...
    open(tLayout) {
      ...
      return this.buildVisual(this.pLayout);
    },
    ...
    moveSprBy(tID, tX, tY) {
      let tsprite = this.pActSprList[tID];
      if (voidp(tsprite)) {
        return _director.error(this, `Sprite not found: ${tID}`, Symbol.for("moveSprBy"), Symbol.for("minor"));
      }
    ...
    moveSprTo(tID, tX, tY) {
      let tsprite = this.pActSprList[tID];
      if (voidp(tsprite)) {
        return _director.error(this, `Sprite not found: ${tID}`, Symbol.for("moveSprTo"), Symbol.for("minor"));
      }
    ...
    activateWrap(tWrapper) {
      let tSpr = tWrapper.getProperty(Symbol.for("sprite"));
    ...
    getPartAtLocation(tLocX, tLocY, tWrapperTypes) {
      ...
      for (const tWrap of this.pWrappedParts) {
        if (tWrapperTypes.getOne(tWrap.getProperty(Symbol.for("type")))) {
          let tPart = tWrap.getPartAt(tLocX, tLocY);
    ...
    createWrapper(tWrapID) {
      if (!voidp(getaProp(this.pWrappedParts, tWrapID))) {
        return _director.error(this, `Duplicate wrap id: ${tWrapID}`, Symbol.for("createWrapper"));
      }
      let tWrap = createObject(Symbol.for("random"), _director.getClassVariable("visualizer.wrapper.class"));
      tWrap.setProperty(Symbol.for("owner"), this.getID());
      this.pWrappedParts[tWrapID] = tWrap;
      let tSpr = sprite(getSpriteManager().reserveSprite(this.getID()));
      tWrap.setProperty(Symbol.for("sprite"), tSpr);
      this.pSpriteList.add(tSpr);
      this.pSpriteData.add(propList());
      return tWrap;
    },

    getWallPartUnderRect(tRect, tSlope) {
      for (const tWrap of this.pWrappedParts) {
        let tWrapType = tWrap.getProperty(Symbol.for("type"));
        if ((tWrapType === Symbol.for("wallleft")) || (tWrapType === Symbol.for("wallright"))) {
          let tPart = tWrap.fitRectToWall(tRect, tSlope);
          if (tPart[Symbol.for("insideWall")] === 1) {
            return tPart;
          }
        }
      }
      return propList({ [Symbol.for("insideWall")]: 0 });
    },
    ...
    setDimmerColor(tColor) {
      ...
      if (_director.memberExists("room_dimmer_image")) {
        let tMem = _director.getMember("room_dimmer_image");
    ...
    Refresh() {
      let tRect = rect(100000, 100000, -100000, -100000);
      for (const tWrapper of this.pWrappedParts) {
        tWrapper.updateWrap();
      }
      ...
      if (this.pSpriteData.count > 0) {
        for (let i = 1; i <= this.pSpriteList.count; i++) {
          if (listp(this.pSpriteData[i])) {
            this.pSpriteData[i][Symbol.for("loc")] = this.pSpriteList[i].loc.__sub__(list([tRect[1], tRect[2]]));
          }
        }
      }
      return 1;
    ...
    buildVisual(tLayout) {
      ...
      if (!listp(tLayout)) {
        return _director.error(this, `Invalid visualizer definition: ${tLayout}`, Symbol.for("buildVisual"), Symbol.for("major"));
      }
      ...
      let tSpriteList = list();
      let tSpriteCollections = propList();
      for (let i = 1; i <= tLayout.count; i++) {
        let tMemNum = _director.getResourceManager().getmemnum(tLayout[i][Symbol.for("member")]);
        if (tMemNum < 1) {
          _director.error(this, `Member ${tLayout[i][Symbol.for("member")]} required by visualizer: ${this.getID()} not found!`, Symbol.for("buildVisual"), Symbol.for("major"));
          continue;
        }
        let tElem = tLayout[i];
        if (!voidp(tElem[Symbol.for("wrapperID")])) {
          let tWrapID = tElem[Symbol.for("wrapperID")];
          if (voidp(this.pWrappedParts[tWrapID])) {
            let tPartWrapper = this.createWrapper(tWrapID);
            let tProps = propList();
          ...
          if (tSpr.spriteNum < 1) {
            ...
            return _director.error(this, "Failed to build visual. System out of sprites!", Symbol.for("buildVisual"), Symbol.for("major"));
          }
          ...
          this.pSpriteData.add(propList());
          tSpriteList.add(tSpr);
        }
        if (!voidp(tElem[Symbol.for("swapAnimType")])) {
          let tAnimProps = propList();
          ...
          _director.error(this, "Animation had no ID", Symbol.for("buildVisual"), Symbol.for("minor"));
        }
      }
      if (tPrivate) {
        ...
        if (tThread !== 0) {
          let tSpr = sprite(getSpriteManager().reserveSprite(this.getID()));
          let tmember = _director.getMember("room_dimmer_image");
          if (tmember !== 0) {
            ...
            tSpr.height = 600;
            tSpr.blend = 100;
            let tGeometry = tThread.getInterface().getGeometry();
            let tScreenLoc = tGeometry.getScreenCoordinate(2, 2, 0);
