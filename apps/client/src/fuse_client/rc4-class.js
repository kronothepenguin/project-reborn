// fuse_client/72_RC4 Class.ls → rc4-class.js
...
export class RC4Class {
  ...
  setKey(tMyKey, tMode) {
    ...
    switch (tMode) {
      ...
      case null:
        for (let i = 0; i <= 255; i++) {
          this.pKey[i + 1] = charToNum(tMyKeyS[(i % tMyKeyS.length)])
          this.pSbox[i + 1] = i
        }
        break
      case symbol('#artificialKey'): {
        let len = bitAnd(tMyKey, 248) / 8
        if (len < 20) len = len + 20
        const tOffset = tMyKey % 1024
        const ckey = []
        for (let i = 0; i < len; i++) {
          const tGiven = this.bitshiftright(tMyKey, i % 32)
          const tOwn = artificialKey[(Math.abs(tOffset + i) % artificialKey.length)]
      ...
      case symbol('#new'):
        ...
        for (let i = 0; i <= 1019; i++) {
          this.pKey[(i % 256) + 1] = (charToNum(tMyKeyS[i % tMyKeyS.length]) + this.pKey[(i % 256) + 1]) % 256
        }
        for (let i = 0; i <= 255; i++) {
          this.pSbox[i + 1] = i
        }
        break
      case symbol('#initPremix'):
        for (let i = 0; i <= 255; i++) {
          this.pKey[i + 1] = charToNum(tMyKeyS[i % tMyKeyS.length])
          this.pSbox[i + 1] = i
        }
    ...
    let j = 0
    for (let i = 0; i <= 255; i++) {
      j = (j + this.pSbox[i + 1] + this.pKey[i + 1]) % 256
      const k = this.pSbox[i + 1]
      this.pSbox[i + 1] = this.pSbox[j + 1]
      this.pSbox[j + 1] = k
    }
    this.i = 0
    this.j = 0

    if (tMode === symbol('#initPremix')) {
  ...
  createKey() {
    ...
    const tSeed = Math.floor(Math.random() * 2147483647)
    Math.seedrandom && Math.seedrandom(theMilliSeconds())
    ...
    Math.seedrandom && Math.seedrandom(tSeed)
    return tTable + tKey
  }
