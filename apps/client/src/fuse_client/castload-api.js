...
function constructCastLoader() {
  return createManager(
    symbol("#castload_manager"),
    getClassVariable("castlib.manager.class"),
  );
}
...
export function startCastLoad(
  ...
  return getCastLoadManager().startCastLoad(
    tCastlibs,
    tPermanentOrNot,
    tAddFlag,
    tDoIndexing,
...
export function registerCastloadCallback(tID, tMethod, tClientObj, tArgument) {
  return getCastLoadManager().registerCallback(
    tID,
    tMethod,
    tClientObj,
    tArgument,
