...
export class DropDownClass {
  ...
  define(tProps) {
    const tField = tProps[sym('#type')] + this.pmodel + '.element'
    this.pProp = getObject(sym('#layout_parser')).parse(tField)
    if (this.pProp === 0) {
      return false
    }
    this.pState = 'close'
    this.pID = tProps[sym('#id')]
    this.pBuffer = tProps[sym('#buffer')]
    this.pSprite = tProps[sym('#sprite')]
    this.pLocX = this.pSprite.left
    this.pLocY = this.pSprite.top
    this.pmodel = tProps[sym('#model')]
    this.pAlignment = tProps[sym('#alignment')]
    this.pTextKeys = tProps[sym('#keylist')]
    this.pOrigWidth = tProps[sym('#width')]
    this.pLineHeight = tProps[sym('#fixedLineSpace')]
    this.pOpenDir = tProps[sym('#direction')]
    this.pMaxWidth = tProps[sym('#maxwidth')]
    this.pLineHeight = tProps[sym('#height')]
    this.pFixedSize = tProps[sym('#fixedsize')]
    this.pOrdering = 1
    if (!voidP(this.pProp[sym('#dropDownType')])) {
      this.pDropDownType = this.pProp[sym('#dropDownType')].getProp(sym('#content'))
    } else {
      this.pDropDownType = sym('#default')
    }
    this.pTextlist = []
    ...
    this.pSprite.blend = tProps[sym('#blend')]
    return true
  }
  ...
  updateData(tTextList, tTextKeys, tChosenIndex, tChosenValue) {
    ...
    this.pDropActiveBtnImg = this.createDropImg([this.pTextlist[this.pSelectedItemNum - 1]], 0, 'up')
    this.pimage = this.pDropActiveBtnImg
    this.render()
    return true
  }
  ...
  setSelection(tSelNumOrStr, tUpdate) {
    ...
    if (tUpdate) {
      this.arrangeTextList(sym('#choose'))
      this.pDropActiveBtnImg = this.createDropImg([this.pTextlist[this.pShowOrder[this.pSelectedItemNum - 1] - 1]], 0, 'up')
      this.pimage = this.pDropActiveBtnImg
  ...
  arrangeTextList(tStyle) {
    if (this.pDropDownType === sym('#titleWithCancel')) {
      switch (tStyle) {
        case sym('#open'):
          if (this.pShowOrder[this.pSelectedItemNum - 1] > 2) {
            this.setShowOrder(sym('#normal'), this.pShowOrder[this.pSelectedItemNum - 1], 1)
          } else {
            this.setShowOrder(sym('#normal'), 1, 2)
          }
          this.pDropMenuImg = this.createDropImg(this.pTextlist, 1, 'up')
          break
        case sym('#choose'):
          if (this.pShowOrder[this.pSelectedItemNum - 1] <= 2) {
            this.setShowOrder(sym('#normal'))
            this.pSelectedItemNum = 1
          }
          break
      }
    }
    if ((this.pDropDownType === sym('#default')) && (this.pOpenDir === sym('#up'))) {
      switch (tStyle) {
        case sym('#open'):
          this.setShowOrder(sym('#normal'), this.pShowOrder[this.pSelectedItemNum - 1])
          this.pDropMenuImg = this.createDropImg(this.pTextlist, 1, 'up')
          break
        case sym('#choose'):
          this.setShowOrder(sym('#normal'), this.pShowOrder[this.pSelectedItemNum - 1], sym('#down'))
          break
      }
    }
    if ((this.pDropDownType === sym('#default')) && (this.pOpenDir === sym('#down'))) {
      switch (tStyle) {
        case sym('#open'):
          this.setShowOrder(sym('#normal'), this.pShowOrder[this.pSelectedItemNum - 1], null, sym('#down'))
          this.pDropMenuImg = this.createDropImg(this.pTextlist, 1, 'up')
          break
        case sym('#choose'):
          this.setShowOrder(sym('#normal'), this.pShowOrder[this.pSelectedItemNum - 1], sym('#down'))
  ...
  openMenu() {
    this.arrangeTextList(sym('#open'))
    this.pimage = this.pDropMenuImg
    this.pLoc = this.pSprite.loc
    switch (this.pOpenDir) {
      case sym('#lastselected'):
        this.pSprite.loc = point(this.pLoc.locH, this.pLoc.locV - ((this.pSelectedItemNum - 1) * this.pLineHeight))
        break
      case sym('#up'):
        this.pSprite.loc = point(this.pLoc.locH, this.pLoc.locV - ((this.pShowOrder.length - 1) * this.pLineHeight))
        break
    }
    this.render()
    this.pState = 'open'
  ...
  chooseFromMenu() {
    ...
    if ((this.pRollOverItem > 0) && (this.pRollOverItem <= this.pShowOrder.length)) {
      ...
      if (!voidP(this.pTextKeys[this.pShowOrder[this.pSelectedItemNum - 1] - 1])) {
        return this.pTextKeys[this.pShowOrder[this.pSelectedItemNum - 1] - 1]
      }
    }
  }
  ...
  UpdateImageObjects(tPalette, tstate) {
    ...
    for (const tV of ['top', 'middle', 'bottom']) {
      for (const tH of ['left', 'middle', 'right']) {
        const tSymbol = sym('#' + tV + tH)
        const tDesc = this.pProp[tstate][sym('#members')][tSymbol]
        const tmember = member(getmemnum(tDesc[sym('#member')]))
        ...
        if (!voidP(tDesc[sym('#rotate')])) {
          this.rotateImg(tImage, tDesc[sym('#rotate')])
        }
        this.pDropDownImg.setaProp(tV + '_' + tH, tImage)
      }
    }
    if (!voidP(this.pProp[sym('#optionalimage')])) {
      const tOptionalImages = this.pProp[sym('#optionalimage')][sym('#members')]
      for (let i = 1; i <= tOptionalImages.count; i++) {
        const tDesc = tOptionalImages[tOptionalImages.getPropAt(i)]
        const tMemName = tDesc[sym('#member')]
