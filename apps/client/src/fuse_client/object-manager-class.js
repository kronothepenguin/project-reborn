...
export class ObjectManagerClass {
  ...
  create(tID, tClassList) {
    ...
    if (objectp(this.pObjectList.getaProp(tID))) {
      return error(this, 'Object already exists: ' + tID, symbol('#create'), symbol('#major'))
    }
    if (tID === symbol('#random')) {
      tID = getUniqueID()
    }
    if (voidP(tClassList)) {
      return error(this, 'Class member name expected!', symbol('#create'), symbol('#major'))
    }
    if (!listp(tClassList)) {
    ...
    for (const tClass of tClassListDup) {
      if (objectp(tClass)) {
        ...
        if (this.managerExists(symbol('#resource_manager'))) {
          tMemNum = this.getManager(symbol('#resource_manager')).getmemnum(tClass)
        } else {
        ...
        if (tMemNum < 1) {
          ...
          return error(this, 'Script not found: ' + tMemNum, symbol('#create'), symbol('#major'))
        }
      ...
      if ((tID !== symbol('#temp')) && (tClassListDup[tClassListDup.length - 1] === tClass)) {
  ...
  registerObject(tID, tObject) {
    if (!objectp(tObject)) {
      return error(this, 'Invalid object: ' + tObject, symbol('#register'), symbol('#major'))
    }
    if (!voidP(this.pObjectList.getaProp(tID))) {
      return error(this, 'Object already exists: ' + tID, symbol('#register'), symbol('#minor'))
    }
    this.pObjectList.setaProp(tID, tObject)
    this.pInstanceList.push(tID)
    return true
  ...
  unregisterObject(tID) {
    if (voidP(this.pObjectList.getaProp(tID))) {
      return error(this, 'Referred object not found: ' + tID, symbol('#unregister'), symbol('#minor'))
    }
    const tObj = this.pObjectList.getaProp(tID)
    this.pObjectList.deleteProp(tID)
    const updateIdx = this.pUpdateList.indexOf(tObj)
  ...
  registerManager(tID) {
    if (!this.exists(tID)) {
      return error(this, 'Referred object not found: ' + tID, symbol('#registerManager'), symbol('#major'))
    }
    if (this.pManagerList.indexOf(tID) >= 0) {
      return error(this, 'Manager already registered: ' + tID, symbol('#registerManager'), symbol('#minor'))
    }
    const instIdx = this.pInstanceList.indexOf(tID)
    if (instIdx >= 0) this.pInstanceList.splice(instIdx, 1)
  ...
  unregisterManager(tID) {
    if (!this.exists(tID)) {
      return error(this, 'Referred object not found: ' + tID, symbol('#unregisterManager'), symbol('#minor'))
    }
    if (this.pInstanceList.indexOf(tID) >= 0) {
      return error(this, 'Manager already unregistered: ' + tID, symbol('#unregisterManager'), symbol('#minor'))
    }
    const mgrIdx = this.pManagerList.indexOf(tID)
    if (mgrIdx >= 0) this.pManagerList.splice(mgrIdx, 1)
    this.pInstanceList.push(tID)
  ...
  getManager(tID) {
    if (!this.pManagerList.includes(tID)) {
      return error(this, 'Manager not found: ' + tID, symbol('#getManager'), symbol('#major'))
    }
  ...
  receivePrepare(tID) {
    ...
    if (!this.pUpdatePause) {
      if (voidP(this.pTimeout)) {
        this.pTimeout = new Timeout('objectmanager' + theMilliSeconds(), 60 * 1000 * 60, () => this.null(), this, null, 0)
        this.pTimeout.start()
      }
    }
    return true
