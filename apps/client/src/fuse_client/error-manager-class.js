...
export class ErrorManagerClass {
  constructor() {
    ...
    this.pErrorLevelList = [symbol('#minor'), symbol('#major'), symbol('#critical')]
    this.pFatalReported = 0
    this.pFatalReportParamOrder = ['error', 'version', 'build', 'os', 'host', 'port', 'client_version', 'mus_errorcode', 'error_id']
  ...
  construct() {
    ...
    if (!variableExists('client.debug.level')) {
      this.pErrorDialogLevel = this.pErrorLevelList[this.pErrorLevelList.length - 1]
    } else {
  ...
  error(tObject, tMsg, tMethod, tErrorLevel) {
    ...
    const tErrorStr = 'the long time' + '-' + tMethod + '-' + tObject + '-' + tMsg.split('\n')[0]
    ...
    if (cacheLines.length > this.pCacheSize) {
      this.pErrorCache = cacheLines.slice(cacheLines.length - this.pCacheSize).join('\n')
    }
  ...
  serverError(tErrorList) {
    if (tErrorList && typeof tErrorList === 'object' && tErrorList[symbol('#errorId')] !== undefined) {
      const tErrorStr = tErrorList[symbol('#errorId')] + '-' + tErrorList[symbol('#errorMsgId')] + '-' + tErrorList[symbol('#time')]
  ...
  makeErrorId() {
    const tSrc = parseInt(getObject(symbol('#session')).GET('user_user_id')) % 10000
  ...
  handleFatalError(tErrorData) {
    ...
    let tParams = ''
    if (!tErrorData || typeof tErrorData !== 'object') {
      this.error(this, 'Invalid error data', symbol('#handleFatalError'), symbol('#major'))
      tErrorData = {}
    }
    ...
    const tConnection = getConnection(getVariable('connection.info.id', symbol('#Info')))
    if (tConnection !== null) {
