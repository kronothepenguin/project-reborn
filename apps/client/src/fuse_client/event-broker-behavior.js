...
export class EventBrokerBehavior {
  ...
  registerProcedure(tMethod, tClientID, tEvent) {
    ...
    if (voidP(tEvent) && voidP(tMethod)) {
      for (let i = 1; i <= this.pProcList.count; i++) {
        this.pProcList._map.set(this.pProcList.getPropAt(i), [this.pProcList.getPropAt(i), tClientID])
      }
    } else {
      if (voidP(tEvent)) {
        for (let i = 1; i <= this.pProcList.count; i++) {
          this.pProcList._map.set(this.pProcList.getPropAt(i), [tMethod, tClientID])
        }
      } else {
        if (voidP(tMethod)) {
          tMethod = tEvent
  ...
  mouseDown() {
    if (!voidP(this.pProcList)) {
      getObject(symbol('#session')).set('client_lastclick', this.id + '->' + this.pProcList[symbol('#mouseDown')][1] + '/' + 'the long time')
    }
