...
export class DownloadInstanceClass {
  ...
  activateWithTimeout() {
    ...
    createTimeout('dl_timeout_' + this.pNetId, tRetryTimeout, symbol('#Activate'), this.pID, null, false)
  }
  ...
  _startFetch() {
    ...
    fetch(this.pURL)
      .then(response => {
        ...
        return response.body.pipeThrough(new TextDecoderStream ? new TextDecoderStream() : undefined)
      ...
      .catch(err => {
        error(this, 'Download error:\n' + this.pMemName + '\n' + err.message + '-' + this.pPercent + 'percent', symbol('#update'), symbol('#minor'))
        this.ptryCount++
        if (this.ptryCount > getIntVariable('download.retry.count', 10)) {
          getDownloadManager().removeActiveTask(this.pMemName, this.pCallBack, false)
          error(this, 'Download failed too many times:\n' + this.pURL + '-' + err.message + '-' + this.pPercent + 'percent', symbol('#update'), symbol('#major'))
        } else {
          if (this.ptryCount > 2) {
