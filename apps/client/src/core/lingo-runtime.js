...
export function ilk(x) {
  ...
  if (typeof x === "object" && x !== null && typeof x.setaProp === "function")
    return "propList";
...
// Lingo: count(x) works on lists and propLists, returns number of items
export function count(x) {
  if (x === undefined || x === null) return 0
  if (Array.isArray(x)) return x.length
  if (typeof x === 'object' && x.count !== undefined) return x.count
...
// Emulates Director's sprite channels. Each channel has a sprite object
// that can be controlled and rendered to the canvas stage.

const _spriteChannels = {};
let _spriteCounter = 0;
const _reservedSprites = new Set();
const MAX_SPRITES = 1000;

export function reserveSprite(clientID) {
  for (let i = 1; i <= MAX_SPRITES; i++) {
    if (!_reservedSprites.has(i)) {
      _reservedSprites.add(i);
      _spriteCounter = Math.max(_spriteCounter, i);
      const sp = createSpriteObject(i);
      _spriteChannels[i] = sp;
...
export function releaseSprite(num) {
  if (_spriteChannels[num]) {
    const sp = _spriteChannels[num];
    sp.visible = false;
    sp.member = null;
    sp.scriptInstanceList = [];
    sp.rect = rect(0, 0, 1, 1);
    sp.locZ = num;
    sp.castNum = 0;
    sp.cursor = 0;
    sp.blend = 100;
...
function createSpriteObject(num) {
  const sp = {
    spriteNum: num,
    member: null,
    castNum: 0,
    ...
    registerProcedure(method, clientID, event) {
      // Placeholder for sprite-level procedure registration
    },
  };
  return sp;
}
...
export function puppetSprite(num, flag) {
  const sp = _spriteChannels[num];
  if (sp) {
    sp.puppet = flag !== false && flag !== 0;
  }
}

export function sprite(num) {
  if (!_spriteChannels[num]) {
    _spriteChannels[num] = createSpriteObject(num);
  }
  return _spriteChannels[num];
}

export function sendSprite(id, msg, ...args) {
  // TODO: dispatch to sprite's scriptInstanceList handlers
}

export function sendAllSprites(msg, ...args) {
  // TODO: dispatch to all sprites
}

// Get all visible sprites sorted by locZ for rendering
export function getVisibleSprites() {
  const sprites = [];
  for (const num in _spriteChannels) {
    const sp = _spriteChannels[num];
    if (sp.visible !== false && sp.member) {
      sprites.push(sp);
    }
  }
  sprites.sort((a, b) => (a.locZ || 0) - (b.locZ || 0));
  return sprites;
}

// Render all visible sprites to the canvas
export function renderSprites(ctx, stageWidth, stageHeight) {
  const sprites = getVisibleSprites();
  for (const sp of sprites) {
    if (!sp.visible || !sp.member) continue;
    ...
    const x = sp.locH || 0;
    const y = sp.locV || 0;
    const w = sp.width || (mem.image ? mem.image.width : 0);
    const h = sp.height || (mem.image ? mem.image.height : 0);

    // Blend/opacity
    if (sp.blend !== undefined && sp.blend < 100) {
      ctx.globalAlpha = sp.blend / 100;
    }

    // Transform (flip, rotation)
    if (sp.flipH || sp.flipV) {
      ctx.translate(x + w / 2, y + h / 2);
      ctx.scale(sp.flipH ? -1 : 1, sp.flipV ? -1 : 1);
    ...
    if (mem.imageUrl) {
      ...
      if (!mem._img) {
        mem._img = new Image();
        mem._img.src = mem.imageUrl;
      }
      if (mem._img.complete && mem._img.naturalWidth > 0) {
        ctx.drawImage(
          mem._img,
          0,
          0,
      ...
      // Member has a canvas (for dynamically created images)
      ctx.drawImage(
        mem._canvas,
        0,
        0,
        w || mem._canvas.width,
        h || mem._canvas.height,
      );
    }
...
// Preload all member images that have imageUrl
export function preloadMemberImages() {
  const promises = [];
  for (const name in _membersByName) {
    const mem = _membersByName[name];
    if (mem.imageUrl && !mem._img) {
      mem._img = new Image();
      mem._img.src = mem.imageUrl;
      promises.push(
        new Promise((resolve) => {
          mem._img.onload = resolve;
          mem._img.onerror = resolve;
        }),
      );
    }
  }
...
export function registerMember(name, num, type, castName, imageUrl) {
  const member = {
    name,
    number: num,
    type,
  ...
  if (imageUrl) {
    member._img = new Image();
    member._img.src = imageUrl;
  }
  return member;
}
